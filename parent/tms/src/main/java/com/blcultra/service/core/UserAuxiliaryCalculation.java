 ...
 */
public class UserAuxiliaryCalculation {

    private Map<String, List<String>> usertactions = new HashMap<>();
    ...
    public UserAuxiliaryCalculation(){
        List<String> public_action = new ArrayList<>(Arrays.asList(ActionEnum.EDIT.getKey(),
                ActionEnum.DELETE.getKey(),ActionEnum.RESETPASS.getKey()));
        usertactions.put(Common.MODULE_USER,public_action);
    ...
    public  List<Map<String, Object>> getActions(String role, String table){
        ...
        List<String> candidateActions = usertactions.get(table);

        for (String candidateAction:candidateActions){
            Map<String, Object> action = actions.get(Common.MODULE_USER+ "_" + candidateAction);
